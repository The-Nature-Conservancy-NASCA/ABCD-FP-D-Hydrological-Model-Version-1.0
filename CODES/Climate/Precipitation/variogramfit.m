function [Rs, S] = variogramfit(h,gammaexp,a0,c0,numobs,varargin)
% -------------------------------------------------------------------------
...
switch lower(params.solver)
    case 'fminsearch'                
        % call fminsearch
        [b,~,exitflag,output] = fminsearch(objectfun,b0,options);
    case 'fminsearchbnd'
        % call fminsearchbnd
        [b,~,exitflag,output] = fminsearchbnd(objectfun,b0,lb,ub,options);
    otherwise
...
% Nash        = 1 - (((S.gamma - S.gammahat).^2)./nanvar(gammaexp));
Rs        = (COVyhaty(2).^2) ./...
              (var(S.gammahat).*var(gammaexp)); % Rsquare
...
if params.plotit
    switch lower(type)
        case 'bounded'
            plot(h,gammaexp,'rs','MarkerSize',5);
            hold on
